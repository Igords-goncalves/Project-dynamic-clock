...
function carregar() {
    ...
    var greet = document.querySelector('div#greet');
    ...
    if (hora >= 0 && hora < 12) {
        ...
        document.body.style.background = 'linear-gradient(to right, #d7e060d0, #e9d736d0), url(./img/bgmorning.jpg)';
        greetings.innerHTML = 'Bom Dia!';
    }
    else if (hora >= 12 && hora < 18) {
        img.src = './gif/sunset.gif'; //O erro está por aqui
        document.body.style.background = 'linear-gradient(to right, #ffc500dc, #c21500dc), url(./img/bgafternoon.jpg)';
        greetings.innerHTML = 'Boa Tarde!';
    }
    else {
        img.src = './gif/night.gif';
        document.body.style.background = 'linear-gradient(to right, #072530d0, #111129d0), url(./img/bgnight.jpg)';
